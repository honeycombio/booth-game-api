...
func (api honeycombQueryDataAPI) callHoneycomb(currentContext context.Context, method string, relativeUrl string, payload []byte) (response []byte, err error) {
...
func (api honeycombQueryDataAPI) CreateQuery(currentContext context.Context, queryDefinition HoneycombQuery, datasetSlug string) (response DefineQueryResponse, err error) {
	...
	bodyBytes, err := api.callHoneycomb(currentContext, "POST", "/queries/"+datasetSlug, queryDefinitionString)
	if err != nil {
		span.RecordError(err)
...
func (api honeycombQueryDataAPI) StartQuery(currentContext context.Context, queryId string, datasetSlug string) (response startQueryResponseBody, err error) {
	...
	startQueryJson, err := api.callHoneycomb(currentContext, "POST", "/query_results/"+datasetSlug, startQueryInputString)
	if err != nil {
		span.RecordError(err)
		span.SetStatus(codes.Error, err.Error())
		return response, err
...
func (api honeycombQueryDataAPI) GiveMeTheData(currentContext context.Context, resultId string, datasetSlug string) (response honeycombQueryData, err error) {
	...
	for {
		pollCount++
		bodyBytes, err := api.callHoneycomb(currentContext, "GET", fmt.Sprintf("/query_results/%s/%s", datasetSlug, resultId), []byte{})
		if err != nil {
			span.RecordError(err)
